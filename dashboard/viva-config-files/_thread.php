...
function _getStatus($s, $e)
{
    ...
    $status = 'unknown';
    if ($e > $now) {
        $status = '<span class="p-2 border rounded-pill bg-info badge">En attente</span>';
    }
    if ($e < $now) {
        $status = '<span class="p-2 border rounded-pill bg-danger badge">Terminée</span>';
    }
    if ($now >= $s && $now <= $e) {
        $status = '<span class="p-2 border rounded-pill bg-success- badge"><span class="spinner-grow spinner-grow-sm mr-1 text-danger"></span>En cours</span>';
    }
    return $status;
}
// var_dump($_SESSION);
